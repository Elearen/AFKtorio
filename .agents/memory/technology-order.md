...
The technology screen and auto-research scheduler use the latest supplied tier list as a flattened canonical order. Normalized catalog aliases are mapped into that list, while catalog entries not present in the list remain available after the supplied order.
