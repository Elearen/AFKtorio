...
**How to apply:** Balance each prototype object, expand helper calls into concrete entries, evaluate numeric expressions safely, preserve boolean and hidden effect fields, canonicalize duplicate names, preserve total per-level unit counts for upgrade families, and validate unique names plus prerequisite references before shipping.